...
from testaaaa import resize_tra
from imggetter import image_get
...
class Application(tk.Frame):
    def __init__(self, master = None):
        ...
        #bottom
        frame_bottom_left=tk.Frame(self.master,borderwidth=2, relief='solid')
        frame_bottom_right=tk.Frame(self.master,borderwidth=2, relief='solid')
        
        #button,label作成
        Label = tk.Label(frame_top,text="webサイトを指定して画像取得",fg='#f00',bg='#000',height=2,width=60)
        button1 = tk.Button(text = "参照",height=2,width=8,command=partial(self.btn_click,"bf"))
        button2 = tk.Button(text = "参照",height=2,width=8,command=partial(self.btn_click,"af"))
        self.Label1 = tk.Label(frame_left,text="前：取得するサイトの画像格納先を選択",fg='#f00',bg='#000',height=2,width=60)
        self.Label2 = tk.Label(frame_left,text="後：取得した画像のリサイズ保存先を選択",fg='#f00',bg='#000',height=2,width=60)
        ...
        for i in range(len(item)):
            tk.Radiobutton(frame_top,value=i,variable=val,text=item[i]).pack(anchor=tk.W)
        ...
        for i in range(len(item2)):
            tk.Radiobutton(frame_bottom_left,value=i+10,variable=val1,text=item2[i]).pack(anchor=tk.W)
